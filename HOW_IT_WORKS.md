...
For each workspace starting with `guest-`, it reads `{workspace}/meta.yaml` to extract the `createdAt` timestamp, adds `guestTTL`, and populates `expiresAt` in the response. The Angular UI uses this to show the countdown timer.
...
Guest workspaces are created without authentication. Five DNS slots (`demo1`–`demo5`) map to pre-created Cloudflare CNAME records pointing at the tunnel. A slot is assigned at workspace creation time, stored in `meta.yaml`, and returned to the client.
